...
- ✅ Обработка голосовых сообщений с транскрибацией речи
...
Бот: 🎤 Распознано: "потратил 500 рублей на кофе"
     Записал ваш расход на кофе в размере 500 рублей.
...
- Максимальная длительность голосового сообщения: 60 секунд (настраивается через `VOICE_MAX_DURATION_SECONDS`)
...
- Для обработки голосовых сообщений требуется OpenAI API ключ (можно использовать тот же, что и для LLM)
