...
# Отложенный импорт ragas, чтобы избежать проблем с Git при старте бота
# ragas импортируется только когда функции evaluation действительно вызываются
...
_ragas_modules = None  # Кеш импортированных модулей ragas
...
def _import_ragas():
    """Отложенный импорт ragas для избежания проблем с Git при старте"""
    global _ragas_modules
    
    # Возвращаем кешированные модули, если уже импортированы
    if _ragas_modules is not None:
        return _ragas_modules
    ...
    try:
        ...
        raise ImportError(
            "Failed to import ragas. This is usually because Git is not installed or not in PATH. "
            "Please install Git from https://git-scm.com/downloads or set GIT_PYTHON_REFRESH=quiet environment variable. "
            f"Original error: {e}"
        )
...
def init_ragas_metrics():
    ...
    metrics = [
        ragas['Faithfulness'](),
        ragas['ResponseRelevancy'](strictness=1),
        ragas['AnswerCorrectness'](),
        ragas['AnswerSimilarity'](),
        ragas['ContextRecall'](),
        ragas['ContextPrecision'](),
    ]
    
    # Инициализируем метрики
    ragas_llm = ragas['LangchainLLMWrapper'](langchain_llm)
    ragas_embeddings = ragas['LangchainEmbeddingsWrapper'](langchain_embeddings)
    
    for metric in metrics:
        if isinstance(metric, ragas['MetricWithLLM']):
            metric.llm = ragas_llm
        if isinstance(metric, ragas['MetricWithEmbeddings']):
...
async def evaluate_dataset(dataset_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    async for result in experiment_results:
        ...
        contexts = [doc.get('page_content', '') for doc in documents if doc.get('page_content')]        
    ...
    # Используем кешированный импорт ragas (уже импортирован в init_ragas_metrics)
    ragas = _import_ragas()
