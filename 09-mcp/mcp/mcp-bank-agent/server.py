...
3. calculate_deposit_profit - расчет доходности вклада с учетом капитализации
...
async def calculate_deposit_profit(
    ...
    - Эффективную ставку
    """
    logger.info(f"calculate_deposit_profit called: amount={initial_amount}, "
                f"rate={annual_rate}%, term={term_months} months, capitalization={capitalization}")
    
    # Конвертируем ставку из процентов в десятичную дробь
    rate_decimal = annual_rate / 100.0
    
    # Срок в годах для расчета
    term_years = term_months / 12.0
    
    if capitalization:
        # С капитализацией: ежемесячное начисление процентов
        # Формула: итоговая = начальная * (1 + годовая_ставка/12)^(срок_в_месяцах)
        monthly_rate = rate_decimal / 12.0
        final_amount = initial_amount * ((1 + monthly_rate) ** term_months)
    else:
        # Без капитализации: простые проценты
        # Формула: итоговая = начальная * (1 + годовая_ставка * срок_в_годах)
        final_amount = initial_amount * (1 + rate_decimal * term_years)
    
    # Рассчитываем доход
    profit = final_amount - initial_amount
    
    # Рассчитываем эффективную ставку (реальная доходность с учетом капитализации)
    effective_rate = ((final_amount / initial_amount) - 1) * 100 / term_years
    
    # Форматируем результат
    result = f"📊 **Расчет доходности вклада**\n\n"
    result += f"**Параметры вклада:**\n"
    result += f"• Начальная сумма: {initial_amount:,.2f} ₽\n"
    result += f"• Годовая ставка: {annual_rate:.2f}%\n"
    result += f"• Срок: {term_months} месяцев ({term_years:.2f} лет)\n"
    result += f"• Капитализация: {'Да' if capitalization else 'Нет'}\n\n"
    
    result += f"**Результаты расчета:**\n"
    result += f"• Итоговая сумма: {final_amount:,.2f} ₽\n"
    result += f"• Полученный доход: {profit:,.2f} ₽\n"
    result += f"• Эффективная ставка: {effective_rate:.2f}% годовых\n\n"
    
    # Дополнительная информация
    if capitalization:
        result += f"💡 **С капитализацией** проценты начисляются ежемесячно и добавляются к сумме вклада, "
        result += f"что увеличивает итоговую доходность.\n"
    else:
        result += f"💡 **Без капитализации** проценты начисляются на начальную сумму вклада.\n"
    
    # Сравнение с капитализацией (если была выбрана без капитализации)
    if not capitalization:
        # Показываем, сколько бы было с капитализацией
        monthly_rate = rate_decimal / 12.0
        final_with_cap = initial_amount * ((1 + monthly_rate) ** term_months)
        profit_with_cap = final_with_cap - initial_amount
        effective_rate_cap = ((final_with_cap / initial_amount) - 1) * 100 / term_years
        
        additional_profit = profit_with_cap - profit
        result += f"\n📈 **Для сравнения:** при капитализации:\n"
        result += f"• Итоговая сумма: {final_with_cap:,.2f} ₽\n"
        result += f"• Доход: {profit_with_cap:,.2f} ₽ (+{additional_profit:,.2f} ₽)\n"
        result += f"• Эффективная ставка: {effective_rate_cap:.2f}% годовых\n"
