...
# Опциональный импорт ragas (может быть не установлен на Python 3.14)
try:
...
if not RAGAS_AVAILABLE:
    logger.warning("ragas is not available. Evaluation features will be disabled. "
                   "Install ragas manually if you need evaluation: pip install ragas>=0.2.0")
...
def create_ragas_embeddings():
    ...
    """
    if not RAGAS_AVAILABLE:
        raise ImportError("ragas is not installed. Install it with: pip install ragas>=0.2.0")
...
async def evaluate_dataset(dataset_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not RAGAS_AVAILABLE:
        raise ImportError(
            "ragas is not installed. Evaluation requires ragas. "
            "Install it with: pip install ragas>=0.2.0\n"
            "Note: ragas may not be compatible with Python 3.14. "
