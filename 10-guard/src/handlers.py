...
# Подавляем ошибку Git для ragas (если Git не установлен)
os.environ.setdefault('GIT_PYTHON_REFRESH', 'quiet')

# Отложенный импорт evaluation (используется только в команде evaluate_dataset)
evaluation = None
...
async def cmd_evaluate_dataset(message: Message):
    ...
    # Отложенный импорт evaluation (загружаем только когда нужно)
    global evaluation
    if evaluation is None:
        try:
            ...
            await message.answer(
                "⚠️ Модуль evaluation недоступен.\n"
                "Возможные причины:\n"
                "• Git не установлен (требуется для ragas)\n"
...
async def handle_message(message: Message):
    ...
    try:
        ...
        if result.get("interrupt"):
            ...
            logger.info(f"🔐 Interrupt detected for chat {message.chat.id}, processing...")
            
            try:
                # Сохраняем interrupt для последующей обработки
                pending_interrupts[message.chat.id] = interrupt_obj
                
                # Безопасное извлечение деталей операции
                tool_name = "unknown"
                tool_args = {}
                
                # Проверяем структуру interrupt объекта
                ...
                # Форматируем сообщение для пользователя (без Markdown для избежания ошибок парсинга)
                interrupt_message = (
                ...
                for key, value in tool_args.items():
                    ...
                    interrupt_message += f"   • {key}: {safe_value}\n"
                ...
                await message.answer(interrupt_message, reply_markup=keyboard)
                logger.info(f"⏸️  Interrupt sent to user {message.chat.id} for tool: {tool_name}")
                return
            except Exception as e:
                logger.error(f"Error processing interrupt for chat {message.chat.id}: {e}", exc_info=True)
                await message.answer(
                    "⚠️ Произошла ошибка при обработке запроса на подтверждение операции. "
